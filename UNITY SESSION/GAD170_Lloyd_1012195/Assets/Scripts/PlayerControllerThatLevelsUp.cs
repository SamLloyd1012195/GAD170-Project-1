...
public class PlayerControllerThatLevelsUp : MonoBehaviour
{
    ...
    {
        currentMoveSpeed = this.moveSpeed + (this.moveSpeed * 8.0f * level);
    ...
    {
        currentTurnSpeed = this.turnSpeed + (this.turnSpeed * (level * 8.0f));
        Debug.Log("currentTurnSpeed = " + currentTurnSpeed);
    }

    void SetCurrentJumpHeight()
    {
        currentJumpHeight = this.jumpHeight + (this.jumpHeight * (level * 5.0f));
